...
def final_model(filename, segSize, nonOL,filter_size):
    ...
    overlap = 50
    model_path = '.\\bp\\bp_main_'+str(64)+'_model_'+str(1280)+'_'+str(nonOL)+'_'+str(filter_size)+'_'+ str(segSize) +'.h5'
    ...
    seq_path = '.\\bp\\bp_seq_'+str(64)+'_model_'+str(1280)+'_'+str(nonOL)+'_'+str(filter_size)+'_'+ str(segSize) +'.h5'
...
def final_return_2_user(inputs_path):
	...
	go_list = pd.read_csv('.\\bp\\bp_go_list2.csv', header=None)
...
def main(input_test_path):
	# CREATING DICTIONARY
	chunkSize = 4
	dict_file = open(".\\bp\\bp_dict.pkl", "rb")
	dict_Prop = pickle.load(dict_file)
