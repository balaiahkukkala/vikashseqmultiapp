...
def final_model(filename, segSize, nonOL,filter_size):
    ...
    overlap = 50
    model_path = '.\\cc\\cc_main_'+str(64)+'_model_'+str(1280)+'_'+str(nonOL)+'_'+str(filter_size)+'_'+ str(segSize) +'.h5'
    ...
    seq_path = '.\\cc\\cc_seq_'+str(64)+'_model_'+str(1280)+'_'+str(nonOL)+'_'+str(filter_size)+'_'+ str(segSize) +'.h5'
...
def final_return_2_user(inputs_path):
    ##predicted results
	final_results = test_fun(inputs_path, 0.62) 
	final_results = final_results.tolist()
	go_list = pd.read_csv('.\\cc\\cc_go_list2.csv', header=None)
...
def main(input_test_path):
	# CREATING DICTIONARY
	chunkSize = 4
	dict_file = open(".\\cc\\cc_dict.pkl", "rb")
	dict_Prop = pickle.load(dict_file)
